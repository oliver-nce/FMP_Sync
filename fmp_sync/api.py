...
"""
API endpoints for FMP Sync app.
"""
...
def toggle_auto_sync(table_names):
	"""
	Toggle the auto_sync_active field for the specified FM Tables.

	Args:
		table_names: List of FM Tables names (or JSON string)
	...
	for table_name in table_names:
		doc = frappe.get_doc("FM Tables", table_name)
...
def get_table_links_grid_data():
	...
	tables = frappe.get_all(
		...
		fields=["frappe_doctype", "fmp_name", "table_name"],
	...
	for row in tables:
		...
		if not dt or dt in seen:
			continue
		seen.add(dt)
		label = row.get("fmp_name") or row.get("table_name") or dt
