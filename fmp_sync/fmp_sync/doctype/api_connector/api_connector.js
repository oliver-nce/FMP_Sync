	...
	refresh(frm) {
		if (!frm.is_new()) {
			frm.add_custom_button(__("Test Connection"), function () {
				frappe.call({
					method: "fmp_sync.fmp_sync.doctype.api_connector.api_connector.test_connection",
...
function add_copy_buttons(frm) {
	CREDENTIAL_FIELDS.forEach(function (fieldname) {
		...
		$btn.on("click", function (e) {
			...
			e.stopPropagation();
			frappe.call({
				method: "fmp_sync.fmp_sync.doctype.api_connector.api_connector.get_credential",
...
function generate_implementation_guide(frm) {
	frappe.call({
		method:
			"fmp_sync.fmp_sync.doctype.api_connector.api_connector.ai_generate_guide",
		args: { connector_name: frm.doc.name },
		freeze: true,
